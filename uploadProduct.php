<?php
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$sellerName = dataFilter($_POST['seller_name']);
		$phone = dataFilter($_POST['phone']);
		$fid = $_SESSION['id'];

		$sql = "INSERT INTO AgroCulture.fproduct (fid, product, pcat, price, seller_name, phone)
				VALUES ('$fid', '$productName', '$productType', '$productPrice', '$sellerName', '$phone')";
		...
		if (in_array($picActualExt, $allowed)) {
			if ($picError === 0) {
				...
				$sql = "UPDATE AgroCulture.fproduct SET picStatus=1, pimage='$picNameNew' WHERE product='$productName';";
		...
		<div class="container">
			<form method="POST" action="uploadProduct.php" enctype="multipart/form-data">
					...
					<div class="col-sm-6">
						<div class="select-wrapper" style="width: auto">
							<select name="type" id="type" required style="background-color:white;color: black;">
								...
								<option value="Grains" style="color: black;">product</option>
								<option value="Grains" style="color: black;">service</option>
							</select>
						</div>
					</div>
					<div class="col-sm-6">
				...
				<div class="row">
					...
					<div class="col-sm-6">
						<input type="text" name="seller_name" id="seller_name" value="" placeholder="Seller Name"
							style="background-color:white;color: black;" />
					</div>
				</div>
				<br>
